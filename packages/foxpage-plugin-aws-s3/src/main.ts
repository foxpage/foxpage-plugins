  ...
  return {
    ...
    visitor: {
      ...
      storageInit: (config: S3ConfigType): void => {
        if (!s3Client || config.newInstance) {
          let proxyConfig = {};
          if (config.proxy) {
          ...
          s3Client = new S3Client(Object.assign({}, config.config, proxyConfig));
      ...
      /**
       * Copy object from bucket1 to bucket2
       * @param options
       */
      async storageCopyObject(options: StorageCopyOptions): Promise<CopyObjectCommandOutput> {
        return s3Client.send(
          new CopyObjectCommand({
            Bucket: options.targetBucket,
            Key: options.targetKey,
            CopySource: options.sourceKey,
          }),
        );
      },
    },
