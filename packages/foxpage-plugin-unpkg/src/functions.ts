 ...
 * @returns Promise
 */
export const getPackageManifest = async (options: { name: string; version: string }): Promise<UnpkgManifest> => {
  try {
    const { data } = await axios({
      method: 'get',
 ...
 * @param  {{name:string;version:string}} options
 */
export const getPackageFoxpage = async (options: { name: string; version: string }): Promise<UnpkgFoxpage> => {
  try {
    const { data } = await axios({
      method: 'get',
 ...
 */
export const formatResponse = (manifestList: UnpkgManifest[], pkgFoxpageData: UnpkgFoxpage[]): IndexContentPkg[] => {
  const resources: IndexContentPkg[] = [];
